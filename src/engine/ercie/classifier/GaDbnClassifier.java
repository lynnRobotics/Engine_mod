package engine.ercie.classifier;
...
import util.algo.Prior;
import util.datastructure.EnvStructure;
import util.datastructure.ExpResult;
	...
	public static SensorNode getSensorNode(String message) {
		...
		if (sensorList.containsKey(s.type + "_" + s.id)) {
			...
			if(s.type.equals("light") && s.discreteValue.equals("on")){
				...
				s.discreteValue = s.discreteValue + "_" + EnvStructure.roomACTargetTemperature.get("livingroom");
			}
			else if(s.name.equals("current_AC_bedroom") && s.discreteValue.equals("on")){
				s.discreteValue = s.discreteValue + "_" + EnvStructure.roomACTargetTemperature.get("bedroom");
			}
